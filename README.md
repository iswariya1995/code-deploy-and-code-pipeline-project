...
This architecture follows AWS best practices for decoupling the development environment from production.

```mermaid
graph LR
    subgraph Development
        Dev[Developer Machine <br/> Amazon Linux 2] --> Root[Project Root Folder]
    end

    subgraph Storage
        Root --> S3[(Artifact Bucket <br/> S3 - Versioning Enabled)]
    end

    subgraph Orchestration
        S3 --> CP_Start
        subgraph CP[AWS CodePipeline]
            CP_Start[Source Stage] --> CP_Build[Build Stage <br/> Skipped]
            CP_Build --> CP_Deploy[Deploy Stage]
        end
    end

    subgraph Production
        CP_Deploy --> CD[AWS CodeDeploy]
        CD --> ALB((Application <br/> Load Balancer))
        
        subgraph VPC[AWS VPC]
            subgraph Subnet[Public Subnets]
                ALB --> EC2_1[Target: Web Server <br/> CodeDeploy Agent]
                ALB --> EC2_2[Target: Web Server <br/> CodeDeploy Agent]
            end
        end
    end
    ...
    subgraph Feedback_Loop
        CD --> CW[CloudWatch]
        CD --> SNS{AWS SNS}
        SNS --> SMS[Smartphone <br/> SMS Alerts]
        SNS --> Email[Email Notifications]
    end

    %% IAM Relations
    IAM[AWS IAM] -.->|s3-ec2-full| Dev
    IAM -.->|s3-ec2-full| S3
    IAM -.->|CodeDeployRole| CD
    IAM -.->|CodeDeployRole| EC2_1
    
    %% Styling
    style Development fill:#fff4dd,stroke:#d4a017
    ...
    style Feedback_Loop fill:#fff3e0,stroke:#e65100
